...
$query = "UPDATE group_messages SET message = ?, file_url = NULL, deleted = 1, deleted_at = ? WHERE id IN ($placeholders)";
...
if ($stmt->execute()) {
    echo json_encode(["status" => "success", "message" => "Group messages marked as deleted"]);
} else {
